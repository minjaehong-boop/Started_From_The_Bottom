...
from restore.workers_batch_test import spawn_workers_batch
def main(cfg_path: str):
    # 1. 설정 파일 로드
    with open(cfg_path, 'r') as f:
        cfg = yaml.safe_load(f)
    batch_cfg = cfg.get("batch_processing", {})
    batch_size = int(batch_cfg.get("batch_size", 8))
    batch_timeout = float(batch_cfg.get("batch_timeout", 0.05))
    ...
    print(f"Spawning 1 batch worker (batch_size={batch_size})...")
    workers = spawn_workers_batch(1, crop_q, done_q, stop_evt, cfg["pipeline_mode"], cfg["kair"], batch_size, batch_timeout)
    detector = None
    if yolo_enabled:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Batch Processing Test with YOLOv8.")
    parser.add_argument("--config", default="configs/default_batch_test.yaml", help="Path to the configuration file.")
